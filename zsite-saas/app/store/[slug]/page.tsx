...
type StoreData={id:string;name:string;slug:string;logo_url:string|null;upi_id:string|null;merchant_name:string|null};
...
export default function Storefront({params}:{params:{slug:string}}){
  ...
  const [customer,setCustomer]=useState({name:'',phone:'',address:'',pincode:'',city:'',state:'',notes:''});
  ...
  const total=cart.reduce((s,l)=>s+Number(l.item.price)*l.quantity,0),count=cart.reduce((s,l)=>s+l.quantity,0);
  ...
  async function placeOrder(){
    ...
    const client=supabase();
    const {error:orderError}=await client.rpc('place_public_order',{p_store_id:store.id,p_customer_name:customer.name,p_customer_phone:customer.phone,p_customer_address:customer.address,p_shipping_pincode:customer.pincode,p_shipping_city:customer.city,p_shipping_state:customer.state,p_notes:customer.notes,p_items:cart.map(l=>({item_id:l.item.id,quantity:l.quantity}))});
  ...
  return <main className="storefront">
    ...
    {checkoutOpen&&<div className="overlay"><section className="checkout-box"><div className="drawer-head"><h2>Checkout</h2><button onClick={()=>setCheckoutOpen(false)}><X/></button></div><p className="checkout-total">Order total <strong>₹{total.toLocaleString('en-IN')}</strong></p><input placeholder="Full name *" value={customer.name} onChange={e=>setCustomer({...customer,name:e.target.value})}/><input placeholder="Phone number *" value={customer.phone} onChange={e=>setCustomer({...customer,phone:e.target.value})}/><textarea placeholder="Delivery address" value={customer.address} onChange={e=>setCustomer({...customer,address:e.target.value})}/><div className="checkout-grid"><input placeholder="Pincode *" value={customer.pincode} onChange={e=>setCustomer({...customer,pincode:e.target.value})}/><input placeholder="City" value={customer.city} onChange={e=>setCustomer({...customer,city:e.target.value})}/><input placeholder="State" value={customer.state} onChange={e=>setCustomer({...customer,state:e.target.value})}/></div><textarea placeholder="Order notes (optional)" value={customer.notes} onChange={e=>setCustomer({...customer,notes:e.target.value})}/><button className="checkout" disabled={!customer.name||!customer.phone||!customer.pincode} onClick={placeOrder}>Place order <ArrowRight size={17}/></button></section></div>}
  </main>
}