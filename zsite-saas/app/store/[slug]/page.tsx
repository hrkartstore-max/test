...
export default function Storefront({params}:{params:{slug:string}}){
  ...
  const subtotal=cart.reduce((s,l)=>s+Number(l.item.price)*l.quantity,0); const discountedSubtotal=Math.max(0,subtotal-discount); const shippingFee=store?.free_shipping_enabled&&subtotal>=Number(store.free_shipping_min_order||0)?0:(selectedCourier?Number(selectedCourier.rate||0):Number(store?.flat_shipping_fee||0)); const codFee=paymentMethod==='cod'?Number(store?.cod_fee||0):0; const total=discountedSubtotal+shippingFee+codFee; const count=cart.reduce((s,l)=>s+l.quantity,0);
