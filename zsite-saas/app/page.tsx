...
function StoreCustomizer({store,setStore}:{store:any;setStore:(v:any)=>void}){
 ...
 <div className="panel"><div className="panelhead">HERO CONTENT</div><div className="field"><label>Hero title</label><input value={form.hero_title} onChange={e=>setForm({...form,hero_title:e.target.value})}/></div><div className="field"><label>Hero subtitle</label><textarea value={form.hero_subtitle} onChange={e=>setForm({...form,hero_subtitle:e.target.value})}/></div><div className="field"><label>Announcement bar</label><input value={form.announcement_text} onChange={e=>setForm({...form,announcement_text:e.target.value})} placeholder="Free shipping on orders above ₹999"/></div><div className="field"><label>Hero image URL</label><input value={form.hero_image_url} onChange={e=>setForm({...form,hero_image_url:e.target.value})} placeholder="https://..."/></div><div className="field"><label>Homepage sections</label><div className="section-list">{[...form.homepage_sections].sort((a:any,b:any)=>a.order-b.order).map((s:any,i:number)=><div className="section-row" key={s.id}><button type="button" onClick={()=>{if(i===0)return;const a=[...form.homepage_sections].sort((x:any,y:any)=>x.order-y.order);[a[i-1].order,a[i].order]=[a[i].order,a[i-1].order];setForm({...form,homepage_sections:a})}}>↑</button><button type="button" onClick={()=>{const a=[...form.homepage_sections].sort((x:any,y:any)=>x.order-y.order);if(i===a.length-1)return;[a[i+1].order,a[i].order]=[a[i].order,a[i+1].order];setForm({...form,homepage_sections:a})}}>↓</button><span>{s.id}</span><label><input type="checkbox" checked={s.enabled} onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,enabled:e.target.checked}:x)})}/> Show</label></div>)}</div></div></div></div>
