...
function StoreCustomizer({store,setStore}:{store:any;setStore:(v:any)=>void}){
 const [form,setForm]=useState<any>({theme_primary:store?.theme_primary||'#f05a3c',theme_background:store?.theme_background||'#f3efe3',theme_text:store?.theme_text||'#111722',logo_url:store?.logo_url||'',theme_font:store?.theme_font||'Inter',theme_button_radius:store?.theme_button_radius||'0px',hero_image_url:store?.hero_image_url||'',homepage_sections:store?.homepage_sections||[{id:'header',enabled:true,order:-1,layout:'logo_cart',show_logo:true,show_cart:true},{id:'announcement',enabled:true,order:0},{id:'hero',enabled:true,order:1},{id:'categories',enabled:true,order:2},{id:'products',enabled:true,order:3},{id:'footer',enabled:true,order:4}],hero_title:store?.hero_title||'Shop your everyday favourites.',hero_subtitle:store?.hero_subtitle||'Simple shopping with direct ordering.',announcement_text:store?.announcement_text||'',header_settings:store?.homepage_sections?.find((x:any)=>x.id==='header')||{id:'header',enabled:true,order:-1,layout:'logo_cart',show_logo:true,show_cart:true},footer_settings:store?.homepage_sections?.find((x:any)=>x.id==='footer')||{id:'footer',enabled:true,order:999,footer_text:'© '+(store?.name||'My Store')+' · All rights reserved.'}});
 const [saving,setSaving]=useState(false);const [saved,setSaved]=useState('');const [uploading,setUploading]=useState('');const [media,setMedia]=useState<any[]>([]);const [mediaLoading,setMediaLoading]=useState(false);const [previewMode,setPreviewMode]=useState<'desktop'|'mobile'>('desktop');
 const templates:any={
   fashion:{name:'Fashion',primary:'#7c3aed',background:'#faf7ff',text:'#17121f',font:'Manrope',radius:'18px',title:'New season. New you.',subtitle:'Curated fashion delivered to your doorstep.',announcement:'FREE SHIPPING ON ORDERS ABOVE ₹999',sections:[
     {id:'announcement',enabled:true,order:0},{id:'hero',enabled:true,order:1},{id:'categories',enabled:true,order:2,title:'SHOP BY CATEGORY'},{id:'featured_products',enabled:true,order:3,title:'TRENDING NOW',limit:8},{id:'image_text',enabled:true,order:4,title:'STYLE THAT FEELS LIKE YOU',heading:'Made for your everyday.',text:'Discover pieces designed for effortless style.',image_url:''},{id:'testimonials',enabled:true,order:5,title:'LOVED BY CUSTOMERS',items:[{author:'Priya',text:'Beautiful quality and fast delivery.'},{author:'Anu',text:'The fit was perfect. Will order again.'}]},{id:'cta',enabled:true,order:6,title:'READY FOR YOUR NEXT LOOK?',heading:'Refresh your wardrobe.',text:'Explore the latest collection.',button:'SHOP NOW',link:'/'} ,{id:'footer',enabled:true,order:7}]},
   electronics:{name:'Electronics',primary:'#2563eb',background:'#f5f8ff',text:'#101827',font:'Inter',radius:'8px',title:'Tech that works for you.',subtitle:'Smart devices, accessories and everyday essentials.',announcement:'FAST DELIVERY • EASY RETURNS • SECURE PAYMENTS',sections:[
     {id:'announcement',enabled:true,order:0},{id:'hero',enabled:true,order:1},{id:'categories',enabled:true,order:2,title:'SHOP BY CATEGORY'},{id:'featured_products',enabled:true,order:3,title:'BEST SELLERS',limit:8},{id:'banner',enabled:true,order:4,title:'POWER UP YOUR EVERYDAY',heading:'Upgrade your setup.',text:'Latest tech at store-friendly prices.',image_url:''},{id:'faq',enabled:true,order:5,title:'FREQUENTLY ASKED QUESTIONS',items:[{question:'Do you offer COD?',text:'COD availability is shown at checkout.'},{question:'How fast is delivery?',text:'Delivery times depend on your pincode.'}]},{id:'cta',enabled:true,order:6,title:'FIND YOUR NEXT DEVICE',heading:'Shop smarter.',button:'BROWSE PRODUCTS',link:'/'} ,{id:'footer',enabled:true,order:7}]},
   restaurant:{name:'Restaurant',primary:'#d97706',background:'#fffaf2',text:'#24180f',font:'Poppins',radius:'8px',title:'Good food. Made fresh.',subtitle:'Order your favourites for pickup or delivery.',announcement:'OPEN TODAY • FRESHLY PREPARED',sections:[
     {id:'announcement',enabled:true,order:0},{id:'hero',enabled:true,order:1},{id:'categories',enabled:true,order:2,title:'EXPLORE OUR MENU'},{id:'featured_products',enabled:true,order:3,title:'CUSTOMER FAVOURITES',limit:8},{id:'image_text',enabled:true,order:4,title:'MADE FRESH',heading:'From our kitchen to your table.',text:'Fresh ingredients, bold flavours and easy ordering.',image_url:''},{id:'testimonials',enabled:true,order:5,title:'WHAT GUESTS SAY',items:[{author:'Rahul',text:'Great taste and quick delivery.'},{author:'Meena',text:'Our new family favourite.'}]},{id:'cta',enabled:true,order:6,title:'HUNGRY?',heading:'Your next meal is one click away.',button:'ORDER NOW',link:'/'} ,{id:'footer',enabled:true,order:7}]},
   optical:{name:'Optical',primary:'#071b49',background:'#f8fafc',text:'#111827',font:'Manrope',radius:'8px',title:'See better. Look better.',subtitle:'Eyewear selected for your everyday vision and style.',announcement:'FREE EYE TEST • EASY RETURNS • SECURE CHECKOUT',sections:[
     {id:'announcement',enabled:true,order:0},{id:'hero',enabled:true,order:1},{id:'categories',enabled:true,order:2,title:'SHOP EYEWEAR'},{id:'featured_products',enabled:true,order:3,title:'TRENDING FRAMES',limit:8},{id:'banner',enabled:true,order:4,title:'YOUR VISION, YOUR STYLE',heading:'Frames made for you.',text:'Discover optical, sunglasses and everyday essentials.',image_url:''},{id:'faq',enabled:true,order:5,title:'OPTICAL FAQ',items:[{question:'Can I add my prescription?',text:'Add prescription details during checkout where supported.'},{question:'Do you offer COD?',text:'COD availability is shown at checkout.'}]},{id:'cta',enabled:true,order:6,title:'FIND YOUR FRAME',heading:'Ready to see your next look?',button:'SHOP EYEWEAR',link:'/'} ,{id:'footer',enabled:true,order:7}]},
   jewellery:{name:'Jewellery',primary:'#a67c00',background:'#fbfaf5',text:'#19160f',font:'Montserrat',radius:'18px',title:'Timeless pieces. Made to shine.',subtitle:'Discover jewellery for every celebration and everyday moment.',announcement:'PACKED WITH CARE • SECURE DELIVERY',sections:[
     {id:'announcement',enabled:true,order:0},{id:'hero',enabled:true,order:1},{id:'categories',enabled:true,order:2,title:'SHOP THE COLLECTION'},{id:'featured_products',enabled:true,order:3,title:'SIGNATURE PIECES',limit:8},{id:'image_text',enabled:true,order:4,title:'MADE TO BE REMEMBERED',heading:'Details that last beyond the moment.',text:'Elegant pieces selected for gifting and everyday luxury.',image_url:''},{id:'testimonials',enabled:true,order:5,title:'CUSTOMER LOVE',items:[{author:'Kavya',text:'Elegant, premium and beautifully packed.'},{author:'Nisha',text:'Looks even better in person.'}]},{id:'cta',enabled:true,order:6,title:'FIND YOUR SIGNATURE PIECE',heading:'Make every moment shine.',button:'SHOP JEWELLERY',link:'/'} ,{id:'footer',enabled:true,order:7}]}
 };
 function applyTemplate(key:string){
   const t=templates[key]; if(!t)return;
   setForm((v:any)=>({...v,theme_primary:t.primary,theme_background:t.background,theme_text:t.text,theme_font:t.font,theme_button_radius:t.radius,hero_title:t.title,hero_subtitle:t.subtitle,announcement_text:t.announcement,homepage_sections:t.sections.map((s:any)=>({...s,items:s.items? s.items.map((x:any)=>({...x})):undefined}))}));
   setSaved(t.name+' template applied. Review and publish changes.');
 } async function uploadAsset(kind:string,file:File){if(!file||!store?.id)return;if(!file.type.startsWith('image/')){setSaved('Please select an image file.');return}if(file.size>5*1024*1024){setSaved('Image must be under 5MB.');return}setUploading(kind);const ext=file.name.split('.').pop()||'jpg';const path=`${store.id}/${kind}-${Date.now()}.${ext}`;const {error}=await supabase().storage.from('store-assets').upload(path,file,{upsert:false,contentType:file.type});if(error){setSaved(error.message);setUploading('');return}const {data}=supabase().storage.from('store-assets').getPublicUrl(path);setForm((v:any)=>({...v,[kind==='logo'?'logo_url':'hero_image_url']:data.publicUrl}));setSaved(`${kind==='logo'?'Logo':'Hero image'} uploaded. Publish changes to apply.`);setUploading('');}
 async function save(){setSaving(true);const sections=[...(form.homepage_sections||[])].filter((x:any)=>x.id!=='header').map((x:any)=>x.id==='footer'?{...x,...(form.footer_settings||{})}:x);sections.unshift({...((form.header_settings)||{id:'header'}),id:'header',order:-1});const payload={...form,homepage_sections:sections};delete payload.header_settings;delete payload.footer_settings;const {data,error}=await supabase().from('stores').update(payload).eq('id',store.id).select().single();if(error)setSaved(error.message);else{setStore(data);setSaved('Store customization saved');}setSaving(false)}
 return <div className="customizer"><div className="pagehead"><div><h1>Store Customizer</h1><p>Customize your storefront brand, hero and announcement without code.</p></div><button className="btn primary" onClick={save} disabled={saving}>{saving?'Saving…':'Publish changes'}</button></div><div className="panel"><div className="panelhead">THEME TEMPLATES <span className="muted">One-click starter design</span></div><div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8}}>{Object.entries(templates).map(([key,t]:any)=><button type="button" key={key} className="template-card" onClick={()=>applyTemplate(key)}><span className="template-swatch" style={{background:t.primary}}></span><b>{t.name}</b><small>{t.font} · {t.radius==='18px'?'Rounded':'Clean'}</small></button>)}</div><small className="muted" style={{display:'block',marginTop:10}}>Templates change colors, typography, hero copy and homepage sections. Your products, categories and media stay untouched.</small></div>
 <div className="panel theme-builder-panel"><div className="panelhead">HEADER & NAVIGATION <span className="muted">Desktop + mobile storefront header</span></div><div className="theme-controls-grid"><div className="field"><label>Header layout</label><select value={form.header_settings?.layout||'logo_cart'} onChange={e=>setForm({...form,header_settings:{...(form.header_settings||{}),layout:e.target.value}})}><option value="logo_cart">Logo + Cart</option><option value="centered">Centered brand</option><option value="compact">Compact</option></select></div><div className="field"><label>Announcement</label><select value={form.header_settings?.announcement_style||'bar'} onChange={e=>setForm({...form,header_settings:{...(form.header_settings||{}),announcement_style:e.target.value}})}><option value="bar">Full bar</option><option value="minimal">Minimal</option><option value="hidden">Hidden</option></select></div><label className="toggle-field"><input type="checkbox" checked={form.header_settings?.show_logo!==false} onChange={e=>setForm({...form,header_settings:{...(form.header_settings||{}),show_logo:e.target.checked}})}/> Show logo</label><label className="toggle-field"><input type="checkbox" checked={form.header_settings?.show_cart!==false} onChange={e=>setForm({...form,header_settings:{...(form.header_settings||{}),show_cart:e.target.checked}})}/> Show cart</label></div></div><div className="panel theme-builder-panel"><div className="panelhead">FOOTER <span className="muted">Store footer content</span></div><div className="theme-controls-grid"><div className="field"><label>Footer text</label><input value={form.footer_settings?.footer_text||''} onChange={e=>setForm({...form,footer_settings:{...(form.footer_settings||{}),footer_text:e.target.value}})} placeholder="© My Store · All rights reserved."/></div><div className="field"><label>Footer alignment</label><select value={form.footer_settings?.align||'center'} onChange={e=>setForm({...form,footer_settings:{...(form.footer_settings||{}),align:e.target.value}})}><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></div></div></div><div className="custom-grid"><div className="panel"><div className="panelhead">BRAND & COLORS</div><div className="custom-fields"><div className="field"><label>Primary color</label><div className="color-input"><input type="color" value={form.theme_primary} onChange={e=>setForm({...form,theme_primary:e.target.value})}/><input value={form.theme_primary} onChange={e=>setForm({...form,theme_primary:e.target.value})}/></div></div><div className="field"><label>Store background</label><div className="color-input"><input type="color" value={form.theme_background} onChange={e=>setForm({...form,theme_background:e.target.value})}/><input value={form.theme_background} onChange={e=>setForm({...form,theme_background:e.target.value})}/></div></div><div className="field"><label>Text color</label><div className="color-input"><input type="color" value={form.theme_text} onChange={e=>setForm({...form,theme_text:e.target.value})}/><input value={form.theme_text} onChange={e=>setForm({...form,theme_text:e.target.value})}/></div></div><div className="field"><label>Font</label><select value={form.theme_font} onChange={e=>setForm({...form,theme_font:e.target.value})}><option>Inter</option><option>Manrope</option><option>Poppins</option><option>Montserrat</option><option>Arial</option></select></div><div className="field"><label>Button corners</label><select value={form.theme_button_radius} onChange={e=>setForm({...form,theme_button_radius:e.target.value})}><option value="0px">Square</option><option value="8px">Rounded</option><option value="18px">Pill</option></select></div></div></div>
 <div className="panel"><div className="panelhead">MEDIA LIBRARY</div><div className="media-library">{mediaLoading?<small>Loading media…</small>:media.length?media.map((m:any)=><div className="media-card" key={m.name}><img src={supabase().storage.from("store-assets").getPublicUrl(`${store.id}/${m.name}`).data.publicUrl} alt=""/><div><b>{m.name.startsWith("logo-")?"Logo":"Hero"}</b><small>{m.name}</small></div><button className="btn" type="button" onClick={()=>useMedia(m.name)}>Use</button><button className="btn" type="button" onClick={()=>removeMedia(m.name)}>Delete</button></div>):<small>No uploaded images yet.</small>}</div></div><div className="panel"><div className="panelhead">BRAND ASSETS</div><div className="asset-upload"><label>Store logo</label>{form.logo_url&&<img className="asset-thumb logo-thumb" src={form.logo_url} alt="Logo preview"/>}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={e=>e.target.files?.[0]&&uploadAsset("logo",e.target.files[0])}/>{uploading==="logo"&&<small>Uploading…</small>}</div><div className="asset-upload"><label>Hero banner</label>{form.hero_image_url&&<img className="asset-thumb hero-thumb" src={form.hero_image_url} alt="Hero preview"/>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>e.target.files?.[0]&&uploadAsset("hero",e.target.files[0])}/>{uploading==="hero"&&<small>Uploading…</small>}</div></div><div className="panel"><div className="panelhead">HERO CONTENT</div><div className="field"><label>Hero title</label><input value={form.hero_title} onChange={e=>setForm({...form,hero_title:e.target.value})}/></div><div className="field"><label>Hero subtitle</label><textarea value={form.hero_subtitle} onChange={e=>setForm({...form,hero_subtitle:e.target.value})}/></div><div className="field"><label>Announcement bar</label><input value={form.announcement_text} onChange={e=>setForm({...form,announcement_text:e.target.value})} placeholder="Free shipping on orders above ₹999"/></div><div className="field"><label>Homepage sections — drag to reorder</label><div className="section-add">{["image_text","banner","featured_products","categories_grid","testimonials","faq","cta"].map((id)=>{const exists=form.homepage_sections.some((x:any)=>x.id===id);return <button type="button" className="btn" key={id} disabled={exists} onClick={()=>setForm({...form,homepage_sections:[...form.homepage_sections,{id,enabled:true,order:form.homepage_sections.length,title:id.replaceAll("_"," ").toUpperCase()}]})}>+ {id.replaceAll("_"," ")}</button>})}</div><div className="section-list">{[...form.homepage_sections].sort((a:any,b:any)=>a.order-b.order).map((s:any,i:number)=>{const key=s.uid||s.id;const sorted=[...form.homepage_sections].sort((a:any,b:any)=>a.order-b.order);const pos=sorted.findIndex((x:any)=>(x.uid||x.id)===key);const core=['announcement','hero','footer'].includes(s.id);const update=(patch:any)=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===key?{...x,...patch}:x)});const move=(dir:number)=>{const a=[...sorted];const to=pos+dir;if(to<0||to>=a.length)return;const [m]=a.splice(pos,1);a.splice(to,0,m);setForm({...form,homepage_sections:a.map((x:any,j:number)=>({...x,order:j}))})};const duplicate=()=>{const copy={...s,uid:'section_'+Date.now()+'_'+Math.random().toString(36).slice(2,7),order:sorted.length,title:(s.title||s.id.replaceAll('_',' '))+' COPY'};setForm({...form,homepage_sections:[...sorted.map((x:any,j:number)=>({...x,order:j})),copy]})};const remove=()=>{if(core)return;setForm({...form,homepage_sections:sorted.filter((x:any)=>(x.uid||x.id)!==key).map((x:any,j:number)=>({...x,order:j}))})};return <div className="section-row" key={key} draggable onDragStart={(e)=>e.dataTransfer.setData('section',key)} onDragOver={e=>e.preventDefault()} onDrop={e=>{const from=e.dataTransfer.getData('section');if(!from||from===key)return;const a=[...sorted];const fi=a.findIndex((x:any)=>(x.uid||x.id)===from);const ti=a.findIndex((x:any)=>(x.uid||x.id)===key);if(fi<0||ti<0)return;const [m]=a.splice(fi,1);a.splice(ti,0,m);setForm({...form,homepage_sections:a.map((x:any,j:number)=>({...x,order:j}))})}}><span className="drag-handle">⋮⋮</span><span className="section-name">{s.id.replaceAll('_',' ')}</span><label><input type="checkbox" checked={s.enabled!==false} onChange={e=>update({enabled:e.target.checked})}/> Show</label>{(s.id==='categories'||s.id==='products'||['image_text','banner','featured_products','categories_grid','testimonials','faq','cta'].includes(s.id))&&<input className="section-title-input" value={s.title||''} placeholder="Section title" onChange={e=>update({title:e.target.value})}/>} {(s.id==='products'||s.id==='featured_products'||s.id==='categories_grid')&&<input className="section-limit-input" type="number" min="1" max="50" value={s.limit||8} onChange={e=>update({limit:Number(e.target.value)})}/>}<div className="section-actions"><button type="button" className="icon-btn" title="Move up" disabled={pos===0} onClick={()=>move(-1)}>↑</button><button type="button" className="icon-btn" title="Move down" disabled={pos===sorted.length-1} onClick={()=>move(1)}>↓</button><button type="button" className="icon-btn" title="Duplicate section" onClick={duplicate}>＋</button><button type="button" className="icon-btn danger" title={core?'Core section cannot be deleted':'Delete section'} disabled={core} onClick={remove}>×</button></div></div>})}</div><div className="section-editor-list">{form.homepage_sections.filter((s:any)=>s.enabled!==false&&['image_text','banner','featured_products','categories_grid','testimonials','faq','cta'].includes(s.id)).map((s:any)=><div className="section-editor" key={s.uid||s.id}><b>{s.id.replaceAll('_',' ').toUpperCase()}</b><span className="muted">Section-specific settings</span><input value={s.title||''} placeholder="Section title" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,title:e.target.value}:x)})}/><input value={s.heading||''} placeholder="Heading" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,heading:e.target.value}:x)})}/><textarea value={s.text||''} placeholder="Description / content" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,text:e.target.value}:x)})}/>{['image_text','banner'].includes(s.id)&&<select value={s.image_url||''} onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,image_url:e.target.value}:x)})}><option value="">Select image from Media Library</option>{media.map((m:any)=><option key={m.name} value={supabase().storage.from('store-assets').getPublicUrl(store.id+'/'+m.name).data.publicUrl}>{m.name}</option>)}</select>} {s.id==='cta'&&<><input value={s.button||''} placeholder="Button text" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,button:e.target.value}:x)})}/><input value={s.link||''} placeholder="Button link e.g. /store/your-slug" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,link:e.target.value}:x)})}/></>} {s.id==='featured_products'&&<input type="number" min="1" max="50" value={s.limit||4} placeholder="Product limit" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,limit:Number(e.target.value)}:x)})}/>} {s.id==='categories_grid'&&<input type="number" min="1" max="50" value={s.limit||8} placeholder="Category limit" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,limit:Number(e.target.value)}:x)})}/>} {s.id==='testimonials'&&<><input value={s.author||''} placeholder="Customer name" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,author:e.target.value}:x)})}/><textarea value={s.text||''} placeholder="Testimonial" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,text:e.target.value}:x)})}/><button type="button" className="btn" onClick={()=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:[...(x.items||[]),{author:'',text:''}]}:x)})}>+ Add testimonial</button>{(s.items||[]).map((it:any,ii:number)=><div className="nested-editor" key={ii}><input value={it.author||''} placeholder="Customer name" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,author:e.target.value}:z)}:x)})}/><textarea value={it.text||''} placeholder="Testimonial" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,text:e.target.value}:z)}:x)})}/><button type="button" className="icon-btn danger" onClick={()=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:(x.items||[]).filter((_:any,j:number)=>j!==ii)}:x)})}>×</button></div>)}</>} {s.id==='faq'&&<><input value={s.question||''} placeholder="Question" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,question:e.target.value}:x)})}/><textarea value={s.text||''} placeholder="Answer" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,text:e.target.value}:x)})}/><button type="button" className="btn" onClick={()=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:[...(x.items||[]),{question:'',text:''}]}:x)})}>+ Add FAQ</button>{(s.items||[]).map((it:any,ii:number)=><div className="nested-editor" key={ii}><input value={it.question||''} placeholder="Question" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,question:e.target.value}:z)}:x)})}/><textarea value={it.text||''} placeholder="Answer" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,text:e.target.value}:z)}:x)})}/><button type="button" className="icon-btn danger" onClick={()=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>(x.uid||x.id)===(s.uid||s.id)?{...x,items:(x.items||[]).filter((_:any,j:number)=>j!==ii)}:x)})}>×</button></div>)}</>}</div>)}</div><small className="muted">Drag sections to change their storefront order. Configure each custom section below.</small></div></div></div>
 <div className="panel custom-preview"><div className="panelhead preview-head"><span>LIVE PREVIEW</span><div className="preview-switch"><button className={previewMode==='desktop'?'active':''} onClick={()=>setPreviewMode('desktop')}>Desktop</button><button className={previewMode==='mobile'?'active':''} onClick={()=>setPreviewMode('mobile')}>Mobile</button></div></div><div className={`preview-frame ${previewMode}`}><div className="preview-store" style={{background:form.theme_background,color:form.theme_text,fontFamily:form.theme_font}}>{form.announcement_text&&<div className="preview-announcement" style={{background:form.theme_primary}}>{form.announcement_text}</div>}<div className="preview-nav"><b>{store?.name||'My Store'}</b><span>Cart</span></div><div className="preview-hero" style={form.hero_image_url?{backgroundImage:`linear-gradient(90deg,${form.theme_background},transparent),url(${form.hero_image_url})`,backgroundSize:"cover"}:undefined}><small>WELCOME TO {(store?.name||'MY STORE').toUpperCase()}</small><h2>{form.hero_title}</h2><p>{form.hero_subtitle}</p><button style={{background:form.theme_primary,color:form.theme_text,borderRadius:form.theme_button_radius}}>Shop now</button></div><div className="preview-section-stack">{[...form.homepage_sections].filter((s:any)=>s.enabled!==false).sort((a:any,b:any)=>a.order-b.order).filter((s:any)=>!["announcement","hero","footer"].includes(s.id)).map((s:any)=><div className="preview-section" key={s.id}><b>{s.title||s.id.replaceAll("_"," ")}</b><small>{s.heading||s.text||"Section preview"}</small></div>)}</div></div></div></div>
