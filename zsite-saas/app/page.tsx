...
function StoreCustomizer({store,setStore}:{store:any;setStore:(v:any)=>void}){
 ...
 <div className="panel"><div className="panelhead">MEDIA LIBRARY</div><div className="media-library">{mediaLoading?<small>Loading media…</small>:media.length?media.map((m:any)=><div className="media-card" key={m.name}><img src={supabase().storage.from("store-assets").getPublicUrl(`${store.id}/${m.name}`).data.publicUrl} alt=""/><div><b>{m.name.startsWith("logo-")?"Logo":"Hero"}</b><small>{m.name}</small></div><button className="btn" type="button" onClick={()=>useMedia(m.name)}>Use</button><button className="btn" type="button" onClick={()=>removeMedia(m.name)}>Delete</button></div>):<small>No uploaded images yet.</small>}</div></div><div className="panel"><div className="panelhead">BRAND ASSETS</div><div className="asset-upload"><label>Store logo</label>{form.logo_url&&<img className="asset-thumb logo-thumb" src={form.logo_url} alt="Logo preview"/>}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={e=>e.target.files?.[0]&&uploadAsset("logo",e.target.files[0])}/>{uploading==="logo"&&<small>Uploading…</small>}</div><div className="asset-upload"><label>Hero banner</label>{form.hero_image_url&&<img className="asset-thumb hero-thumb" src={form.hero_image_url} alt="Hero preview"/>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>e.target.files?.[0]&&uploadAsset("hero",e.target.files[0])}/>{uploading==="hero"&&<small>Uploading…</small>}</div></div><div className="panel"><div className="panelhead">HERO CONTENT</div><div className="field"><label>Hero title</label><input value={form.hero_title} onChange={e=>setForm({...form,hero_title:e.target.value})}/></div><div className="field"><label>Hero subtitle</label><textarea value={form.hero_subtitle} onChange={e=>setForm({...form,hero_subtitle:e.target.value})}/></div><div className="field"><label>Announcement bar</label><input value={form.announcement_text} onChange={e=>setForm({...form,announcement_text:e.target.value})} placeholder="Free shipping on orders above ₹999"/></div><div className="field"><label>Homepage sections — drag to reorder</label><div className="section-add">{["image_text","banner","featured_products","categories_grid","testimonials","faq","cta"].map((id)=>{const exists=form.homepage_sections.some((x:any)=>x.id===id);return <button type="button" className="btn" key={id} disabled={exists} onClick={()=>setForm({...form,homepage_sections:[...form.homepage_sections,{id,enabled:true,order:form.homepage_sections.length,title:id.replaceAll("_"," ").toUpperCase()}]})}>+ {id.replaceAll("_"," ")}</button>})}</div><div className="section-list">{[...form.homepage_sections].sort((a:any,b:any)=>a.order-b.order).map((s:any,i:number)=><div className="section-row" key={s.id} draggable onDragStart={(e)=>e.dataTransfer.setData('section',s.id)} onDragOver={e=>e.preventDefault()} onDrop={e=>{const from=e.dataTransfer.getData('section');if(!from||from===s.id)return;const a=[...form.homepage_sections].sort((x:any,y:any)=>x.order-y.order);const fi=a.findIndex((x:any)=>x.id===from),ti=a.findIndex((x:any)=>x.id===s.id);const [m]=a.splice(fi,1);a.splice(ti,0,m);setForm({...form,homepage_sections:a.map((x:any,j:number)=>({...x,order:j}))})}}><span className="drag-handle">⋮⋮</span><span className="section-name">{s.id}</span><label><input type="checkbox" checked={s.enabled!==false} onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,enabled:e.target.checked}:x)})}/> Show</label>{(s.id==='categories'||s.id==='products'||s.id==='image_text'||s.id==='banner'||s.id==='featured_products'||s.id==='categories_grid'||s.id==='testimonials'||s.id==='faq'||s.id==='cta')&&<input className="section-title-input" value={s.title||''} placeholder={s.id==='categories'?'Categories title':'Products title'} onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,title:e.target.value}:x)})}/>} {s.id==='products'&&<input className="section-limit-input" type="number" min="1" max="50" value={s.limit||8} onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,limit:Number(e.target.value)}:x)})}/>}</div>)}</div><div className="section-editor-list">{form.homepage_sections.filter((s:any)=>s.enabled!==false&&['image_text','banner','featured_products','categories_grid','testimonials','faq','cta'].includes(s.id)).map((s:any)=><div className="section-editor" key={s.id}><b>{s.id.replaceAll('_',' ').toUpperCase()}</b><input value={s.title||''} placeholder="Section title" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,title:e.target.value}:x)})}/><input value={s.heading||''} placeholder="Heading" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,heading:e.target.value}:x)})}/><textarea value={s.text||''} placeholder="Description / content" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,text:e.target.value}:x)})}/>{['image_text','banner'].includes(s.id)&&<select value={s.image_url||''} onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,image_url:e.target.value}:x)})}><option value="">Select image from Media Library</option>{media.map((m:any)=><option key={m.name} value={supabase().storage.from('store-assets').getPublicUrl(store.id+'/'+m.name).data.publicUrl}>{m.name}</option>)}</select>} {s.id==='cta'&&<input value={s.link||''} placeholder="Button link e.g. /store/your-slug" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,link:e.target.value}:x)})}/>} {s.id==='featured_products'&&<input type="number" min="1" max="50" value={s.limit||4} placeholder="Product limit" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,limit:Number(e.target.value)}:x)})}/>} {s.id==='categories_grid'&&<input type="number" min="1" max="50" value={s.limit||8} placeholder="Category limit" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,limit:Number(e.target.value)}:x)})}/>} {s.id==='testimonials'&&<><input value={s.author||''} placeholder="Customer name" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,author:e.target.value}:x)})}/><textarea value={s.text||''} placeholder="Testimonial" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,text:e.target.value}:x)})}/><button type="button" className="btn" onClick={()=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,items:[...(x.items||[]),{author:'',text:''}]}:x)})}>+ Add testimonial</button>{(s.items||[]).map((it:any,ii:number)=><div className="nested-editor" key={ii}><input value={it.author||''} placeholder="Customer name" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,author:e.target.value}:z)}:x)})}/><textarea value={it.text||''} placeholder="Testimonial" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,text:e.target.value}:z)}:x)})}/></div>)}</>} {s.id==='faq'&&<><input value={s.question||''} placeholder="Question" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,question:e.target.value}:x)})}/><textarea value={s.text||''} placeholder="Answer" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,text:e.target.value}:x)})}/><button type="button" className="btn" onClick={()=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,items:[...(x.items||[]),{question:'',text:''}]}:x)})}>+ Add FAQ</button>{(s.items||[]).map((it:any,ii:number)=><div className="nested-editor" key={ii}><input value={it.question||''} placeholder="Question" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,question:e.target.value}:z)}:x)})}/><textarea value={it.text||''} placeholder="Answer" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,items:(x.items||[]).map((z:any,j:number)=>j===ii?{...z,text:e.target.value}:z)}:x)})}/></div>)}</>}} {s.id==='cta'&&<input value={s.button||''} placeholder="Button text" onChange={e=>setForm({...form,homepage_sections:form.homepage_sections.map((x:any)=>x.id===s.id?{...x,button:e.target.value}:x)})}/>}</div>)}</div><small className="muted">Drag sections to change their storefront order. Configure each custom section below.</small></div></div></div>
