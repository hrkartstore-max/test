...
const sb=publicSupabase();const store=await sb.from("stores").select("id,name").eq("slug",String(req.params.slug)).eq("published",true).maybeSingle();
if(store.error)throw store.error;if(!store.data)return res.status(404).json({message:"Store not found or not published"});
