...
app.get("/api/subscription", requireUser, async(req,res)=>{try{const store=await getOwnedStore(req as AuthRequest);const sb=tenantStore(req as AuthRequest);let {data,error}=await sb.from("store_subscriptions").select("*").eq("store_id",store.id).maybeSingle();if(error)throw error;if(!data){const created=await sb.from("store_subscriptions").insert({store_id:store.id,plan:"starter",billing_interval:"monthly",provider:"cashfree",status:"initialized",amount:200}).select("*").single();if(created.error)throw created.error;data=created.data;}res.json({subscription:{planId:data.plan==="starter"?"starter":data.plan,status:data.status==="active"?"active":"trialing",renewsAt:data.current_period_end},plan:{id:data.plan,name:data.plan==="starter"?"Basic":data.plan==="growth"?"Growth":"Pro+"}});}catch(e:any){res.status(500).json({message:e.message});}});
...
app.post("/api/subscription/checkout", requireUser, async(req,res)=>{res.json({ok:true,mode:"local",message:"Subscription checkout is ready for payment provider configuration."});});
app.get("/api/plans",(_req,res)=>res.json([{id:"starter",name:"Basic",price:200,interval:"month"},{id:"pro",name:"Pro",price:999,interval:"month"},{id:"business",name:"Pro+",price:1999,interval:"month"}]));
