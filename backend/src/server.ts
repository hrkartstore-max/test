...
if(created.error)throw created.error;const oi=await sb.from("order_items").insert(orderItems.map(x=>({...x,order_id:created.data.id})));if(oi.error)throw oi.error;\nfor(const x of orderItems){const current=await adminSupabase().from("items").select("stock").eq("id",x.item_id).single();if(current.error)throw current.error;const next=Math.max(0,Number(current.data.stock||0)-x.quantity);const upd=await adminSupabase().from("items").update({stock:next}).eq("id",x.item_id);if(upd.error)throw upd.error;}
