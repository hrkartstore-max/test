...
app.use(express.json({ limit: "8mb", verify: (req, _res, buf) => { if ((req as any).originalUrl === "/api/payments/cashfree/webhook") (req as any).rawBody = Buffer.from(buf); } }));
...
function productOut(p: any) {
  return {
    _id: p.id, id: p.id, name: p.name, slug: p.id, sku: p.sku || "",
...
app.get("/api/notifications", requireUser, async (req:AuthRequest,res:Response)=>{try{const sb=tenantStore(req);const {data,error}=await sb.from("notification_logs").select("id,order_id,channel,event,recipient,message,status,provider_message_id,error,created_at").order("created_at",{ascending:false}).limit(100);if(error)throw error;res.json(data||[])}catch(e:any){res.status(500).json({message:e.message})}});

app.get("/api/inventory/history", requireUser, async(req,res)=>{try{const store=await getOwnedStore(req as AuthRequest);if(!store)return res.status(404).json({message:"Store not found"});const sb=tenantStore(req as AuthRequest);const {data,error}=await sb.from("inventory_stock_history").select("id,item_id,previous_stock,new_stock,change,reason,created_at,items(name,sku)").eq("store_id",store.id).order("created_at",{ascending:false}).limit(100);if(error)throw error;res.json(data||[])}catch(e:any){res.status(500).json({message:e.message})}});

app.patch("/api/products/:id/stock", requireUser, async(req,res)=>{const p=z.object({stock:z.coerce.number().int().min(0),reason:z.string().max(200).optional()}).safeParse(req.body);if(!p.success)return res.status(400).json({message:"Stock must be a non-negative integer"});try{const store=await getOwnedStore(req as AuthRequest);if(!store)return res.status(404).json({message:"Store not found"});const sb=tenantStore(req as AuthRequest);const item=await sb.from("items").select("id,name,stock").eq("id",req.params.id).eq("store_id",store.id).maybeSingle();if(item.error)throw item.error;if(!item.data)return res.status(404).json({message:"Product not found"});const updated=await sb.from("items").update({stock:p.data.stock}).eq("id",item.data.id).select("id,name,stock").single();if(updated.error)throw updated.error;const history=await sb.from("inventory_stock_history").insert({store_id:store.id,item_id:item.data.id,previous_stock:Number(item.data.stock||0),new_stock:p.data.stock,change:p.data.stock-Number(item.data.stock||0),reason:p.data.reason||"manual adjustment"});if(history.error)throw history.error;res.json({ok:true,product:updated.data,previousStock:item.data.stock,reason:p.data.reason||"manual adjustment"});}catch(e:any){res.status(500).json({message:e.message});}});
...
app.post("/api/products", requireUser, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const { data, error } = await tenantStore(req).from("items").insert({
      store_id: store.id, name: p.data.name, sku: p.data.sku || null, description: p.data.description || "",
...
app.patch("/api/products/:id", requireUser, async(req,res)=>{try{const store=await getOwnedStore(req as AuthRequest);if(!store)return res.status(404).json({message:"Store not found"});const stock=Number(req.body?.stock);if(!Number.isInteger(stock)||stock<0)return res.status(400).json({message:"Stock must be a non-negative integer"});const sb=tenantStore(req as AuthRequest);const current=await sb.from("items").select("id,stock").eq("id",String(req.params.id)).eq("store_id",store.id).maybeSingle();if(current.error)throw current.error;if(!current.data)return res.status(404).json({message:"Product not found"});const r=await sb.from("items").update({stock}).eq("id",current.data.id).eq("store_id",store.id).select("*").single();if(r.error)throw r.error;const history=await sb.from("inventory_stock_history").insert({store_id:store.id,item_id:current.data.id,previous_stock:Number(current.data.stock||0),new_stock:stock,change:stock-Number(current.data.stock||0),reason:String(req.body?.reason||"manual adjustment")});if(history.error)throw history.error;res.json(productOut(r.data));}catch(e:any){res.status(500).json({message:e.message});}});
...
app.post("/api/public/stores/:slug/payment", async(req,res)=>{
const p=z.object({customer:z.object({name:z.string().min(2),phone:z.string().min(8),email:z.string().email(),address:z.string().optional()}),items:z.array(z.object({productId:z.string(),quantity:z.coerce.number().int().min(1)})).min(1)}).safeParse(req.body);
if(!p.success)return res.status(400).json({message:"Valid customer email and order details are required"});
try{const secret=process.env.CASHFREE_CLIENT_SECRET,id=process.env.CASHFREE_CLIENT_ID;if(!secret||!id)return res.status(503).json({message:"Online payment is not configured"});
const sb=publicSupabase();const store=await sb.from("stores").select("id,name,cod_enabled,upi_enabled").eq("slug",String(req.params.slug)).eq("published",true).maybeSingle();
if(store.error)throw store.error;if(!store.data)return res.status(404).json({message:"Store not found or not published"});const sub=await adminSupabase().from("store_subscriptions").select("plan,status").eq("store_id",store.data.id).maybeSingle();if(sub.error)throw sub.error;if(sub.data?.status!=="active"||sub.data.plan!=="pro")return res.status(402).json({message:"Online payments require the PRO plan.",code:"PLAN_FEATURE",feature:"online_gateway",plan:sub.data?.plan||"free"});
const ids=p.data.items.map(x=>x.productId);const ps=await sb.from("items").select("*").in("id",ids).eq("store_id",store.data.id).eq("active",true);if(ps.error)throw ps.error;
const map=new Map((ps.data||[]).map(x=>[x.id,x]));const orderItems=p.data.items.map(x=>{const item=map.get(x.productId);if(!item)throw new Error("One or more products are unavailable");const stock=Number(item.stock ?? 0);if(stock<x.quantity)throw new Error(item.name+" has only "+stock+" left in stock");const price=Number(item.compare_at_price ?? item.price);return {item_id:item.id,item_name:item.name,quantity:x.quantity,unit_price:price,total:price*x.quantity};});
const total=orderItems.reduce((s,x)=>s+x.total,0);const created=await sb.from("orders").insert({store_id:store.data.id,customer_name:p.data.customer.name,customer_phone:p.data.customer.phone,customer_address:p.data.customer.address||"",status:"new",payment_status:"pending",subtotal:total,discount:0,total,payment_method:"cashfree"}).select("*").single();
if(created.error)throw created.error;const oi=await sb.from("order_items").insert(orderItems.map(x=>({...x,order_id:created.data.id})));if(oi.error)throw oi.error;
const orderId="hepord_"+created.data.id.replaceAll("-","").slice(0,20)+"_"+Date.now();const base=process.env.FRONTEND_URL||"https://test-2f5l.vercel.app";
const cf=await fetch("https://api.cashfree.com/pg/orders",{method:"POST",headers:{"Content-Type":"application/json","x-api-version":"2025-01-01","x-client-id":id,"x-client-secret":secret},body:JSON.stringify({order_id:orderId,order_amount:total,order_currency:"INR",customer_details:{customer_id:created.data.id,customer_name:p.data.customer.name,customer_phone:p.data.customer.phone,customer_email:p.data.customer.email},order_meta:{return_url:base+"/store/"+req.params.slug+"?payment=processing&order_id="+orderId,notify_url:base+"/api/payments/cashfree/webhook"},order_note:"HEPRA store order "+created.data.id})});
const data=await cf.json();if(!cf.ok)throw new Error(data.message||"Cashfree order creation failed");await sb.from("orders").update({payment_reference:orderId}).eq("id",created.data.id);
res.status(201).json({mode:"cashfree",orderId,paymentSessionId:data.payment_session_id,orderNumber:"HP-"+created.data.id.slice(0,8).toUpperCase(),total});
}catch(e:any){res.status(500).json({message:e.message||"Unable to start payment"});}});

app.post("/api/public/stores/:slug/orders", async(req,res)=>{const p=z.object({customer:z.object({name:z.string().min(2),phone:z.string().min(8),email:z.string().email().optional(),address:z.string().optional()}),items:z.array(z.object({productId:z.string(),quantity:z.coerce.number().int().min(1)})).min(1),couponCode:z.string().optional()}).safeParse(req.body);if(!p.success)return res.status(400).json({message:"Invalid order details"});try{const sb=publicSupabase();const store=await sb.from("stores").select("id").eq("slug",String(req.params.slug)).eq("published",true).maybeSingle();if(store.error)throw store.error;if(!store.data)return res.status(404).json({message:"Store not found or not published"});const ids=p.data.items.map(x=>x.productId);const ps=await sb.from("items").select("*").in("id",ids).eq("store_id",store.data.id).eq("active",true);if(ps.error)throw ps.error;const map=new Map((ps.data||[]).map(x=>[x.id,x]));const orderItems=p.data.items.map(x=>{const item=map.get(x.productId);if(!item)throw new Error("One or more products are unavailable");const price=Number(item.compare_at_price ?? item.price);return {item_id:item.id,item_name:item.name,quantity:x.quantity,unit_price:price,total:price*x.quantity};});const subtotal=orderItems.reduce((s,x)=>s+x.total,0);let discount=0;if(p.data.couponCode){const c=await sb.from("discounts").select("*").eq("store_id",store.data.id).eq("code",p.data.couponCode.toUpperCase()).eq("active",true).maybeSingle();if(c.data){discount=c.data.kind==="fixed"?Number(c.data.value):subtotal*Number(c.data.value)/100;discount=Math.min(discount,subtotal);}}const total=Math.max(0,subtotal-discount);const created=await sb.from("orders").insert({store_id:store.data.id,customer_name:p.data.customer.name,customer_phone:p.data.customer.phone,customer_address:p.data.customer.address||"",status:"new",payment_status:"pending",subtotal,discount,total,payment_method:"cod",discount_code:p.data.couponCode?.toUpperCase()||null,discount_amount:discount}).select("*").single();if(created.error)throw created.error;const oi=await sb.from("order_items").insert(orderItems.map(x=>({...x,order_id:created.data.id})));if(oi.error)throw oi.error;const stockResult=await adminSupabase().rpc("adjust_order_stock",{p_order_id:created.data.id});if(stockResult.error){await adminSupabase().from("orders").delete().eq("id",created.data.id);return res.status(409).json({message:stockResult.error.message||"Insufficient stock"});}await logNotification(store.data.id,created.data.id,"order_placed",p.data.customer.phone,"Order placed successfully. Order: HP-"+created.data.id.slice(0,8).toUpperCase());res.status(201).json({orderNumber:`HP-${created.data.id.slice(0,8).toUpperCase()}`,subtotal,discount,total,status:created.data.status});}catch(e:any){res.status(500).json({message:e.message});}});

app.get("/api/public/stores/:slug/orders/track", async(req,res)=>{const phone=String(req.query.phone||"").trim();const orderNumber=String(req.query.order||"").trim().replace(/^HP-/i,"");if(phone.length<8||orderNumber.length<4)return res.status(400).json({message:"Order number and phone are required"});try{const sb=publicSupabase();const store=await sb.from("stores").select("id").eq("slug",String(req.params.slug)).eq("published",true).maybeSingle();if(store.error)throw store.error;if(!store.data)return res.status(404).json({message:"Store not found"});const q=await sb.from("orders").select("id,status,payment_status,total,created_at,customer_phone,stores!inner(slug,name)").eq("store_id",store.data.id).eq("customer_phone",phone).ilike("id",orderNumber+"%").maybeSingle();if(q.error)throw q.error;if(!q.data)return res.status(404).json({message:"Order not found"});res.json({orderNumber:"HP-"+q.data.id.slice(0,8).toUpperCase(),status:q.data.status,paymentStatus:q.data.payment_status,total:q.data.total,createdAt:q.data.created_at});}catch(e:any){res.status(500).json({message:e.message});}});
