...
}catch(e:any){res.status(500).json({message:e.message||"Unable to start payment"});}});    await logNotification(store.data.id, order.id, "order_placed", customer.phone, "Order placed successfully. Order: HP-"+order.id.slice(0,8).toUpperCase());
